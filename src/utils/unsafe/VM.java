...
/*
 * Copyright (c) 1996, 2013, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
...
public class VM {
 ...
 /* The following methods used to be native methods that instruct the VM to selectively suspend certain threads in low-memory situations. They are inherently dangerous and not implementable on native
  * threads. We removed them in JDK 1.2. The skeletons remain so that existing applications that use these methods will still work.
  */
 ...
 /*
  * Not supported in 1.2 because these will have to be exported as JVM functions, and we are not sure we want do that. Leaving here so it can be easily resurrected -- just remove the // comments.
  */
 /**
  * Resume Java profiling. All profiling data is added to any earlier profiling, unless <code>resetJavaProfiler</code> is called in between. If profiling was not started from the command line,
  * <code>resumeJavaProfiler</code> will start it.
  * <p>
  *
  * NOTE: Profiling must be enabled from the command line for a java.prof report to be automatically generated on exit; if not, writeJavaProfilerReport must be invoked to write a report.
  * <p>
 ...
 /**
  * Initialize Java profiling. Any accumulated profiling information is discarded.
  */
    // public native static void resetJavaProfiler();
 /**
  * Write the current profiling contents to the file "java.prof". If the file already exists, it will be overwritten.
  */
 ...
 // behaviour.
 //
 private static final boolean defaultAllowArraySyntax = false;
 ...
 /**
  * Returns the system property of the specified key saved at system initialization time. This method should only be used for the system properties that are not changed during runtime. It accesses a
  * private copy of the system properties so that user's locking of the system properties object will not cause the library to deadlock.
  * <p>
  * Note that the saved system properties do not include the ones set by sun.misc.Version.init().
  * <p>
  * @param key
  * <p>
  * @return
 ...
 /* The threadStatus field is set by the VM at state transition in the hotspot implementation. Its value is set according to the JVM TI specification GetThreadState function.
  */
 ...
 /*
  * Returns the first non-null class loader up the execution stack, or null if only code from the null class loader is on the stack.
  */
