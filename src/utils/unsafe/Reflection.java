...
/*
 * Copyright (c) 2001, 2013, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
...
public class Reflection {
 ...
 /** Used to filter out fields and methods from certain classes from public view, where they are sensitive or they may contain VM-internal objects. These Maps are updated very rarely. Rather than
  * synchronize on each access, we use copy-on-write */
 ...
 /** Retrieves the access flags written to the class file. For inner classes these flags may differ from those returned by Class.getModifiers(), which searches the InnerClasses attribute to find the
  * source-level access flags. This is used instead of Class.getModifiers() for run-time access checks due to compatibility reasons; see 4471811. Only the values of the low 13 bits (i.e., a mask of
  * 0x1FFF) are guaranteed to be valid.
  * <p>
  * @param c
  * <p>
  * @return */
 ...
 private static Member[] filter ( Member[] members, String[] filteredNames ) {
  ...
  for ( Member member : members ) {
   ...
   for ( String filteredName : filteredNames ) {
    if ( member.getName().equals(filteredName) ) {
     shouldSkip = true;
     break;
    }
   }
 ...
 public static boolean isCallerSensitive ( Method m ) {
  ...
  if ( utils.unsafe.VM.isSystemDomainLoader(loader) || isExtClassLoader(loader) ) {
